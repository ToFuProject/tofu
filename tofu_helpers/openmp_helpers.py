...
def check_for_openmp():
    ...
    flag_omp = _get_openmp_flag(compiler)
...
def is_openmp_installed():
    """ Returns ``True`` if OpenMP is installed, else ``False``
    and generates ``tofu.openmp_enabled.is_openmp_enabled``, which can then be used
    to determine, post build, whether the package was built with or without
    OpenMP support."""

    openmp_support = not check_for_openmp()
    generate_openmp_enabled_py(openmp_support)
