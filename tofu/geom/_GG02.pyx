...
def Poly_isClockwise(cnp.ndarray[double,ndim=2] Poly):
    ...
    # cdef cnp.ndarray[int,ndim=2] wh_ymin = np.where(Poly[1,:] == Poly[1,:].min())
    # cdef int idmin = wh_ymin[0][np.argmax(Poly[0,wh_ymin])]
    # cdef idm1  =  idmin - 1
    # cdef idp1  = (idmin + 1) % (NP + 1)
    # Sum = Poly[0, idm1] * (Poly[1, idmin] - Poly[1, idp1]) + \
    #   Poly[0, idmin] * (Poly[1, idp1] - Poly[1, idm1]) + \
    #   Poly[0, idp1] * (Poly[1, idm1] - Poly[1, idmin])
    return Sum < 0.
