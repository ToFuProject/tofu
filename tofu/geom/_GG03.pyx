            ...
            if (val_a * val_a < eps_a * eps_a):
                if (val_b * val_b < eps_b * eps_b):
                    ...
                    if coeff * coeff < eps_a * eps_a :
                        ...
                        # no intersection between Cone and LOS, which means that
                        # ray director vector and cone generatrix are parallel
                        # but also that u_Z is = 0. This case was already tested, so
                        # there is problem in algorithm
                        with gil:
                            print(" ******* ERROR !!!!!!!!!!!!!! uz = 0 ? *********** ")
