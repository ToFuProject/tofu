...
void sum_columns_blocks(double *orig, double *out, int n_rows, int n_cols){
   int n_blocks=n_cols/MAX;
   int b;
   int j;
   int i;
   for (b=0; b<n_blocks; b++){   //for every block
   ...
   //left_overs:
   int left=n_cols-n_blocks*MAX;
   double res[MAX]={0.0};         //initialize to 0.0
   for(j=0;j<n_rows;j++){
       for(i=0;i<left;i++){   //calculate sum for left rows simultaniously
              res[i]+=orig[(n_blocks*MAX)*n_rows+j];
       }
   }
   for(i=0;i<left;i++){
         out[n_blocks*MAX+i]=res[i];
