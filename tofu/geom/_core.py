                ...
                for kk in self._dStruct["dObj"][k].keys():
                    for pp in self._dextraprop["lprop"]:
                        setattr(
                            ...
                            "set_%s" % pp,
                            lambda val, pk=pp, k0=k, k1=kk: self._set_extraprop(
                                pk, val, k0, k1
                            ),
                        )
                        setattr(
...
class Rays(utils.ToFuObject):
    ...
    def _checkformat_inputs_dgeom(self, dgeom=None):
        ...
        lC = [
            k
            for k in self._dcases.keys()
            if (
                isinstance(dgeom, self._dcases[k]["type"])
        ...
        if dgeom["case"] in ["A", "B", "C"]:
            ...
            if np.all(vect2[indok] < 1.0e-9):
                ...
                if np.all(scaabs < 1.0e-9):
                    ...
                    try:
                        x1, x2, n1, n2, ind1, ind2, indr = self._get_x12_fromflat(
                            x12
                        )
                        dgeom["dX12"].update(
                            {
    ...
    def _kInOut_Isoflux_inputs_usr(self, lPoly, lVIn=None):
        ...
        if lVIn is None:
            ...
            for ii in range(0, nPoly):
                lVIn[ii] = (
                    lVIn[ii] / np.sqrt(np.sum(lVIn[ii] ** 2, axis=0))[None, :]
                )
    ...
    def calc_signal(
        ...
        if newcalc:
            ...
            for ii in range(0, self.nRays):
                sig[:, ii] = (
                    np.nansum(val[:, indpts[ii] : indpts[ii + 1]], axis=-1)
                    * reseff[ii]
                )
