...
# #############################################################################
# #############################################################################
#                           Mesh2D - time selection
# #############################################################################


def get_hastime_tindt(
    coll=None,
    key=None,
    t=None,
    indt=None,
    ...
    # indt
    if indt is not None:
        indt = np.atleast_1d(indt).ravel()

    # t vs indt
    if t is not None and indt is not None:
        msg = "Please provide t xor indt, not both!"
        raise Exception(msg)

    # dim
    dim = _generic_check._check_var(
        dim, 'dim',
        types=str,
        default='time',
    )
    ...
    hastime = None
    kbs = coll.ddata[key]['bsplines']
    if kbs in coll.dobj.get('bsplines', {}).keys():
        refbs = coll.dobj['bsplines'][kbs]['ref']

        hastime = (
            len(coll.ddata[key]['ref']) == len(refbs) + 1
            and coll.ddata[key]['ref'][1:] == refbs
        )
    ...
    if len(lt) == 0:
        if hastime is True:
            msg = (
                f"key {key} was found to have a time dimension, "
                "but no corresponding time vector could be identified!"
            )
            warnings.warn(msg)
        elif hastime is None:
            hastime = False
        ...
        if hastime is False:
            msg = (
                "Contradiction:\n"
                f"\t- {key} should not have time dimension\n"
                f"\t- time vector {keyt} identified"
            )
            raise Exception(msg)
        elif hastime is None:
            hastime = True
        ...
        if hastime is False:
            msg = (
                "Contradiction:\n"
                f"\t- {key} should not have time dimension\n"
                f"\t- Several time vectors identified: {lt}"
            )
            raise Exception(msg)
        else:
            msg = (
    ...
    if hastime:
        if keyt is None:
            ...
            reft = coll.ddata[keyt]['ref'][0]
            axist = coll.ddata[key]['ref'].index(reft)

        nt = coll.ddata[key]['data'].shape[axist]

    # --------
    # t vs indt

    if (t is not None or indt is not None) and not hastime :
        msg = (
            "Contradtiction:\n"
            f"\t- '{key}' does not seem to have a time dimension\n"
            "=> Args t and indt cannot be provided"
        )
        raise Exception(msg)
    ...
    if hastime and t is not None:
        if keyt is None:
            msg = (
                "Contradiction:\n"
                f"'{key}' has no valid time vector identified\n"
                "=> arg t cannot be provided!"
            )
            raise Exception(msg)

        t0 = coll.ddata[keyt]['data']
        t0bins = 0.5*(t0[1:] + t0[:-1])
        indt = np.digitize(t, t0bins)
    elif not hastime:
        t = None
    ...
    if indt is not None:
        if indt.dtype == np.bool_:
            if indt.size != nt:
                msg = f"indt as bool must have shape ({nt},), not {indt.shape}"
                raise Exception(msg)
        elif indt.dtype == np.int_:
            if np.nanmax(indt) >= nt:
                msg = f"indt as int must be < {nt}\nProvided: {indt}"
                raise Exception(msg)
        else:
            msg = (
                "Arg indt must be provided as bool or int array of indices!\n"
                f"Provided: {indt}"
            )
            raise Exception(msg)
    ...
    if indt is not None:
        ...
        if indtu.size < indt.size:
            indt_reverse = np.array(
                [indt == iu for iu in indtu],
                dtype=bool,
            )

    if indt_reverse is None:
        indtu = None

    return hastime, keyt, reft, nt, t, indt, indtu, indt_reverse
