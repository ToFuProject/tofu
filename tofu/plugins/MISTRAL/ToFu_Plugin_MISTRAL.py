...
def Create_Ves(Name='V1', shot=None):
    PathInp, PathObj, PathOut = TFPF.get_DefaultPaths(Exp=Exp)
    if Name=='V1':
        PolyRef = np.loadtxt(PathInp+'/'+Exp+'_Ves.txt', dtype='float', comments='#', delimiter=None, converters=None, skiprows=0, usecols=None, unpack=False, ndmin=2)
    Ves = TFG.Ves(Name, PolyRef, Exp=Exp, shot=shot, Type='Lin', DLong=[0.,1.])
    Ves.save()
    return Ves


def _rescale_Pos(Pos):
    if np.any(Pos[:2,:]>1000):
        Pos[:2,:] = Pos[:2,:]/10000.
    elif np.any(Pos[:2,:]>100):
        Pos[:2,:] = Pos[:2,:]/1000.
    elif np.any(Pos[:2,:]>10):
        Pos[:2,:] = Pos[:2,:]/100.
    return Pos


def Create_GLOS(shot=None):
    PathInp, PathObj, PathOut = TFPF.get_DefaultPaths(Exp=Exp)
    # Get detector position and orientation
    Pos = np.loadtxt(PathInp+'/Sensor_position.txt', dtype='float', comments='#', delimiter=None, converters=None, skiprows=0, usecols=None, unpack=False, ndmin=2)
    Pos = _rescale_Pos(Pos)

    # Load Ves
    Ves = TFPF.Open(PathObj+'/TFG_VesLin_MISTRAL_V1_sh0.npz')

    # Create LOS and assemble
    indH, indV = range(16,32), range(0,16)
    LLOSH, LLOSV = [], []
    for ii in range(0,Pos.shape[1]):
        D = np.concatenate(([0.5],Pos[:2,ii]))
        uu = np.concatenate(([0.],[np.cos(Pos[2,ii])],[np.sin(Pos[2,ii])]))
        Nb = indH.index(ii)+1 if ii in indH else indV.index(ii)+1
        Name = 'H_{0:02.0f}'.format(Nb) if ii in indH else 'V_{0:02.0f}'.format(Nb)
        ll = TFG.LOS(Name, (D,uu), Ves=Ves, Diag='Vis', shot=shot)
        if ii in indH:
            LLOSH.append(ll)
        elif ii in indV:
            LLOSV.append(ll)
    GLosH = TFG.GLOS('H', LLOSH, Exp='MISTRAL', Diag='Vis', shot=None)
    GLosV = TFG.GLOS('V', LLOSV, Exp='MISTRAL', Diag='Vis', shot=None)

    return GLosH, GLosV



def Create_LensDet(shot=None, Detect=True, GDetect=True, save=False):
    PathInp, PathObj, PathOut = TFPF.get_DefaultPaths(Exp=Exp)
    # Get detector position and orientation
    Pos = np.loadtxt(PathInp+'/Sensor_position.txt', dtype='float', comments='#', delimiter=None, converters=None, skiprows=0, usecols=None, unpack=False, ndmin=2)
    Pos = _rescale_Pos(Pos)
    ...
    # Get Lens parameters
    Par = np.loadtxt(PathInp+'/Lens_Param.txt', dtype='float', comments='#', delimiter=None, converters=None, skiprows=0, usecols=None, unpack=False, ndmin=1)
    ...
    for ii in range(0,Pos.shape[1]):
        ...
        Lens = TFG.Lens(Name, D, uu, float(Par[0]), float(Par[1]), F2=np.inf, Type='Sph', Ves=Ves, Clock=False, Diag='Vis', shot=shot, R1=None, R2=None, dd=None)
        if ii in indH:
            LLensH.append(Lens)
        elif ii in indV:
            LLensV.append(Lens)
        ...
        if Detect:
            Type = 'Circ'
            Det = TFG.Detect(Name, {'Rad':400.e-6}, Type='Circ', Optics=Lens, Ves=Ves, shot=shot, Clock=False, Diag='Vis', CalcEtend=True, CalcSpanImp=True, CalcCone=True, CalcPreComp=True, Calc=True,
                    Etend_Method='quad', Etend_dX12=[0.01,0.01], Etend_dX12Mode='rel',
                    Cone_DX=0.0005, Cone_DY=0.0005, Cone_DZ=0.0005, Verb=True)
            if ii in indH:
                LDetH.append(Det)
            elif ii in indV:
                LDetV.append(Det)
    if GDetect:
        GDetH, GDetV = None, None
        if len(LDetH)>0:
            GDetH = TFG.GDetect('H', LDetH, Exp='MISTRAL', Diag='Vis', shot=0)
        if len(LDetV)>0:
            GDetV = TFG.GDetect('V', LDetV, Exp='MISTRAL', Diag='Vis', shot=0)
    ...
    if GDetect:
        ...
        return LLensH, LLensV




def Calc_Res(inds=range(0,50), width=1000):
    GDet = TFPF.Open('/afs/ipp-garching.mpg.de/home/d/didiv/Python/tofu/src/Objects_MISTRAL/TFG_GDetect_MISTRAL_DgVis_Full_sh0.npz')
    SavePath = '/afs/ipp-garching.mpg.de/home/d/didiv/ForOthers/ForPierre/'
    Out = np.load(SavePath+'PtsForRes.npz')
    ...
    for ii in range(0,len(inds)):
        print inds[ii]
        if inds[ii]<NP/width:
            Lind.append(width*inds[ii] + np.arange(0,width))
        else:
            Lind.append(width*inds[ii] + np.arange(0,NP%width))
        oo = GDet.calc_Res(Pts=Out['Pts'][:,Lind[-1]], steps=0.00025, Thres=0.1, IntResCross=[0.05,0.05], save=True, SavePath=SavePath, SaveName='Res_width{0:04.0f}_{1:02.0f}'.format(width,inds[ii]))

def Load_Res(width=1000):
    SavePath = '/afs/ipp-garching.mpg.de/home/d/didiv/ForOthers/ForPierre/'
    Out = np.load(SavePath+'PtsForRes.npz')
    NP = Out['Pts'].shape[1]

    inds = range(0,NP/width)
    Out = {'ThresMode':[], 'Res':[], 'IntResLongMode':[], 'IntResLong':[], 'ThresMin':[], 'LDetLim':[], 'IntResCross':[], 'IntResCrossMode':[], 'steps':[], 'Mode':[], 'Thres':[], 'Pts':[]}
    for ii in range(0,len(inds)):
        rr = np.load(SavePath+'Res_width{0:04.0f}_{1:02.0f}.npz'.format(width,inds[ii]))
        for kk in Out.keys():
            Out[kk].append(rr[kk])

    Out['Pts'] = np.concatenate(tuple(Out['Pts']), axis=1)
    Out['Res'] = np.concatenate(tuple(Out['Res']))
    Out['LDetLim'] = list(np.concatenate(tuple(Out['LDetLim'])))
    assert all([Out['steps'][ii]==Out['steps'][0] for ii in range(0,len(inds))])
    Out['steps'] = float(Out['steps'][0])
    assert all([Out['Mode'][ii]==Out['Mode'][0] for ii in range(0,len(inds))])
    Out['Mode'] = str(Out['Mode'][0])
    assert all([Out['ThresMode'][ii]==Out['ThresMode'][0] for ii in range(0,len(inds))])
    Out['ThresMode'] = str(Out['ThresMode'][0])
    assert all([Out['IntResLongMode'][ii]==Out['IntResLongMode'][0] for ii in range(0,len(inds))])
    Out['IntResLongMode'] = str(Out['IntResLongMode'][0])
    assert all([Out['IntResCrossMode'][ii]==Out['IntResCrossMode'][0] for ii in range(0,len(inds))])
    Out['IntResCrossMode'] = str(Out['IntResCrossMode'][0])
    assert all([np.all(Out['Thres'][ii]==Out['Thres'][0]) for ii in range(0,len(inds))])
    Out['Thres'] = Out['Thres'][0]
    assert all([Out['ThresMin'][ii]==Out['ThresMin'][0] for ii in range(0,len(inds))])
    Out['ThresMin'] = float(Out['ThresMin'][0])
    assert all([Out['IntResLong'][ii]==Out['IntResLong'][0] for ii in range(0,len(inds))])
    Out['IntResLong'] = float(Out['IntResLong'][0])
    assert all([np.all(Out['IntResCross'][ii]==Out['IntResCross'][0]) for ii in range(0,len(inds))])
    Out['IntResCross'] = list(Out['IntResCross'][0])
