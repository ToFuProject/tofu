...
def dumpro(filename, w_dir, shot_name, vid = True, rate = None, 
    ...
    The vid parameter is set to be True by default, but the code automatically
    sets its value depending on the input
    ...
    vid:              True
     A parameter that specifies whether the input is a video or image files
    tlim:             tuple
    ...
    #check background subtraction rate
    if rate == None:
    ...
    if vid == True:
        ...
        im_path, m_data, s_name = vid2img.video2img(filename, w_dir, 
                                                    shot_name, meta_data, verb)
    
        im_col['original'] = im_path
    else:
        #if input is a collection of image
        #saving path of original image
        im_col['original'] = filename
