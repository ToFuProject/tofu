...
def Mesh1D_Plot(Knots, Cents=None, y=0., Leg=None, Elt='KCN', ax=None, Kdict=tfd.M1Kd, Cdict=tfd.M1Cd, LegDict=tfd.TorLegd, draw=True, a4=False, Test=True):
    if Test:
        assert isinstance(Knots,np.ndarray) and Knots.ndim==1, "Arg Knots must be a 1-dim np.ndarray !"
        assert type(y) is float, "Arg y must be a float !"
        assert type(Leg) is str, "Arg Leg must be a str !"
        assert ax is None or isinstance(ax,plt.Axes), "Arg ax must be a plt.Axes instance !"
        assert type(Kdict) is dict, "Arg Kdict must be a dict !"
        assert type(Cdict) is dict, "Arg Cdict must be a dict !"
        assert type(Elt) is str, "Arg Elt must be a str !"

    if ax is None:
        ax = tfd.Plot_Mesh1D_DefAxes(a4=a4)
    if 'K' in Elt:
        ax.plot(Knots, y*np.ones(Knots.shape),label=Leg + ' Knots', **Kdict)
    if 'C' in Elt:
        if Cents is None:
            Cents = (Knots[:-1]+Knots[1:])/2.
        ax.plot(Cents, y*np.ones(Cents.shape), label=Leg + ' Cents', **Cdict)
    if 'N' in Elt:
        YLim = ax.get_ylim()
        DY = YLim[1]-YLim[0]
        for ii in range(Cents.size):
            ax.annotate(str(ii),xy=(Cents[ii], y), xycoords='data', xytext=(Cents[ii], y+0.04*DY), textcoords='data', ha='center', va='bottom')
    Delta = np.abs(np.max(Knots)-np.min(Knots))
    ax.set_xlim([np.min(Knots)-0.02*Delta, np.max(Knots)+0.02*Delta])
    ax.set_yticks([])
...
def Mesh1D_Plot_Res(Knots, Leg=None, ax=None, Dict=tfd.M1Resd, LegDict=tfd.TorLegd, draw=True, a4=False, Test=True):
    if Test:
        assert isinstance(Knots,np.ndarray) and Knots.ndim==1, "Arg Knots must be a 1-dim np.ndarray !"
        assert type(Leg) is str, "Arg Leg must be a str !"
        assert ax is None or isinstance(ax,plt.Axes), "Arg ax must be a plt.Axes instance !"
        assert type(Dict) is dict, "Arg dict must be a dict !"

    if ax is None:
        ax = tfd.Plot_Res_Mesh1D_DefAxes(a4=a4)
    Res = np.diff(Knots)
    Eps = np.min(Res)/1000.
    ResX = np.array([Knots[:-1]+Eps, Knots[1:]-Eps]).T
    ResY = np.array([Res,Res]).T
    ax.plot(ResX.flatten(), ResY.flatten(), label=Leg+' Res', **Dict)
    if not LegDict is None:
        ax.legend(**LegDict)
    if draw:
        ax.figure.canvas.draw()
...
def Mesh2D_Plot(M2, ax=None, Leg=None, Elt='MBgKCBsBv', indKnots=None, indCents=None, SubMesh=None,
                Bckdict=tfd.M2Bckd, Mshdict=tfd.M2Mshd, Kdict=tfd.M2Kd, Cdict=tfd.M2Cd, LegDict=tfd.TorLegd,
                draw=True, a4=False, Test=True):

    if Test:
        assert M2.Id.Cls=='Mesh2D', "Arg M2 must be a Mesh2D instance !"
        assert ax is None or isinstance(ax,plt.Axes), "Arg ax must be a plt.Axes instance !"
        assert all([type(dd) is dict for dd in [Bckdict,Mshdict,Kdict,Cdict]]), "Arg Bckdict,Mshdict,Kdict,Cdict must be dict !"
        assert type(LegDict) is dict or LegDict is None, "Arg LegDict must be a dict !"
        assert type(Elt) is str, "Arg Elt must be a str !"

    if not SubMesh is None:
        M2 = M2._SubMesh[SubMesh]['Mesh2D']

    # Preparing
    if indKnots is None:
        indKnots = np.zeros((M2.NKnots,),dtype=bool)
    elif indKnots=='all':
        indKnots = np.ones((M2.NKnots,),dtype=bool)
    elif hasattr(indKnots,'__getitem__') and all(type(ii) in [int,np.int64] for ii in indKnots):
        indKnots = np.asarray(indKnots)
        ind = np.zeros((M2.NKnots,),dtype=bool)
        ind[indKnots] = True
        indKnots = ind
    else:
        assert isinstance(indKnots,np.ndarray) and indKnots.dtype == bool, "Arg indKnots must be None, 'all' or a list/tuple/np.ndarray of int indices or a np.ndarray of bool !"

    if indCents is None:
        indCents = np.zeros((M2.NCents,),dtype=bool)
    elif indCents=='all':
        indCents = np.ones((M2.NCents,),dtype=bool)
    elif hasattr(indCents,'__getitem__') and all(type(ii) in [int,np.int64] for ii in indCents):
        indCents = np.asarray(indCents)
        ind = np.zeros((M2.NCents,),dtype=bool)
        ind[indCents] = True
        indCents = ind
    else:
        assert isinstance(indCents,np.ndarray) and indCents.dtype == bool, "Arg indCents must be None, 'all' or a list/tuple/np.ndarray of int indices or a np.ndarray of bool !"
    indKnotsEx = np.zeros((M2.NKnots,),dtype=bool)
    indKnotsEx[np.unique(M2._Cents_Knotsind[:,indCents])] = True
    indCentsEx  = np.zeros((M2.NCents,),dtype=bool)
    indtemp = np.unique(M2._Knots_Centsind[:,indKnots])
    indCentsEx[indtemp[indtemp>=0]] = True

    # Plotting
    Leg = M2.Id.NameLTX if Leg is None else Leg
    if ax is None:
        ax = tfd.Plot_Mesh2D_DefAxes(VType=M2.Type, a4=a4)

    NR, NZ = M2.MeshX1.NKnots, M2.MeshX2.NKnots
    if 'Bg' in Elt:
        Rmin, Rmax = M2.MeshX1.Knots.min(), M2.MeshX1.Knots.max()
        Zmin, Zmax = M2.MeshX2.Knots.min(), M2.MeshX2.Knots.max()
        RH = np.tile(np.array([Rmin,Rmax,np.nan]),(NZ))
        ZH = np.concatenate((np.tile(M2.MeshX2.Knots,(2,1)),np.nan*np.ones((1,NZ))),axis=0).T.flatten()
        RV = np.concatenate((np.tile(M2.MeshX1.Knots,(2,1)),np.nan*np.ones((1,NR))),axis=0).T.flatten()
        ZV = np.tile(np.array([Zmin,Zmax,np.nan]),(NR))
        RB, ZB = np.concatenate((RH,RV)), np.concatenate((ZH,ZV))
        ax.plot(RB, ZB, label=Leg+' Background', **Bckdict)

    if 'M' in Elt:
        ROR = np.concatenate((np.tile(M2.MeshX1.Knots,(2,1)),np.nan*np.ones((1,NR))),axis=0).T.flatten()
        ZOR = np.nan*np.ones((3,NR))
        for ii in range(0,NR):
            indK = M2.Knots[0,:] == M2.MeshX1.Knots[ii]
            ZOR[0,ii] = M2.Knots[1,indK].min()
            ZOR[1,ii] = M2.Knots[1,indK].max()
        ZOR = ZOR.T.flatten()
        ZOZ = np.concatenate((np.tile(M2.MeshX2.Knots,(2,1)),np.nan*np.ones((1,NZ))),axis=0).T.flatten()
        ROZ = np.nan*np.ones((3,NZ))
        for ii in range(0,NZ):
            indK = M2.Knots[1,:] == M2.MeshX2.Knots[ii]
            ROZ[0,ii] = M2.Knots[0,indK].min()
            ROZ[1,ii] = M2.Knots[0,indK].max()
        ROZ = ROZ.T.flatten()
        RO, ZO = np.concatenate((ROR,ROZ)), np.concatenate((ZOR,ZOZ))
        ax.plot(RO, ZO, label=Leg+' Mesh', **Mshdict)

    if 'K' in Elt:
        if not indKnots is None and np.any(indKnots):
            ll = ax.plot(M2.Knots[0,indKnots], M2.Knots[1,indKnots], label=Leg+' Knots', **Kdict)
        if not indKnotsEx is None and np.any(indKnotsEx):
            kd = Kdict.copy()
            kd['alpha'] = 0.2
            ax.plot(M2.Knots[0,indKnotsEx], M2.Knots[1,indKnotsEx], label=Leg+' Knots of Cents', **kd)
    if 'C' in Elt:
        if not indCents is None and np.any(indCents):
            ax.plot(M2.Cents[0,indCents], M2.Cents[1,indCents], label=Leg+' Cents', **Cdict)
        if not indCentsEx is None and np.any(indCentsEx):
            cd = Cdict.copy()
            cd['alpha'] = 0.2
            ax.plot(M2.Cents[0,indCentsEx], M2.Cents[1,indCentsEx], label=Leg+' Cents of Knots', **cd)
    if 'Bs' in Elt:
        ax.plot(M2.BaryS[0],M2.BaryS[1], ls='None', lw=0., marker='o', c='k', label=Leg+' BaryS')
    if 'Bv' in Elt and M2.Id.Type=='Tor':
        ax.plot(M2.BaryV[0],M2.BaryV[1], ls='None', lw=0., marker='.', c='k', label=Leg+' BaryV')
...
def Mesh2D_Plot_Res(M2, ax1=None, ax2=None, ax3=None, axcb=None, Leg=None, Elt='MBgS', SubMesh=None, Bckdict=tfd.M2Bckd, Mshdict=tfd.M2Mshd, Sdict=tfd.M2Sd, LegDict=tfd.M2Legd, draw=True, a4=False, Test=True):
    if Test:
        assert M2.Id.Cls=='Mesh2D', "Arg M2 must be a Mesh2D instance !"
        assert all([ax is None or isinstance(ax,plt.Axes) for ax in [ax1,ax2,ax3,axcb]]), "Arg ax1,ax2,ax3,axcb must be a plt.Axes instance !"
        assert all([type(dd) is dict for dd in [Bckdict,Mshdict]]), "Args Bckdict, Mshdict must be dict !"
        assert type(LegDict) is dict or LegDict is None, "Arg LegDict must be a dict !"
        assert type(Elt) is str, "Arg Elt must be a str !"
        assert not ('S' in Elt and 'V' in Elt), "Cannot plot simultaneously the surface and the volume !"
    ...
    if None in [ax1,ax2,ax3,axcb]:
        ax1, ax2, ax3, axcb = tfd.Plot_Res_Mesh2D_DefAxes(a4=a4, VType=M2.Id.Type)

    if 'M' in Elt or 'B' in Elt:
        ax1 = Mesh2D_Plot(M2, ax=ax1, Leg=Leg, Elt=Elt, Bckdict=Bckdict, Mshdict=Mshdict, LegDict=None, draw=False, a4=False, Test=False)

    # Plotting surface resolution
    if 'S' in Elt or 'V' in Elt:
        assert not (M2.Id.Type=='Lin' and 'V' in Elt), "Cannot plot VolAng if not a 'Tor' Type !"
        patch = []
        quant = M2.Surfs*1e4 if 'S' in Elt else M2.VolAngs*1e6
        ylab = r"S ($cm^2$)" if 'S' in Elt else r"VolAng ($cm^3/sr$)"
        for ii in range(0,M2.NCents):
            Rk = np.unique(M2.Knots[0,M2._Cents_Knotsind[:,ii]])
            Zk = np.unique(M2.Knots[1,M2._Cents_Knotsind[:,ii]])
            patch.append(patches.Polygon(np.array([[Rk[0], Zk[0]], [Rk[1], Zk[0]], [Rk[1], Zk[1]], [Rk[0], Zk[1]]]), True))
        ppp = PcthColl(patch, **Sdict)
        ppp.set_array(quant)
        CNb = ax1.add_collection(ppp)
        cbar = plt.colorbar(CNb, cax=axcb)
        axcb.set_ylabel(ylab)

    # Plotting 1D resolution
    ResRB, ResRM = np.diff(M2.MeshX1.Knots), np.diff(np.unique(M2.Knots[0,:]))
    ResZB, ResZM = np.diff(M2.MeshX2.Knots), np.diff(np.unique(M2.Knots[1,:]))
    Eps = min(np.min(ResRB),np.min(ResZB))/1000.

    ResRXB = np.array([M2.MeshX1.Knots[:-1]+Eps, M2.MeshX1.Knots[1:]-Eps]).T
    ResZXB = np.array([M2.MeshX2.Knots[:-1]+Eps, M2.MeshX2.Knots[1:]-Eps]).T
    ResRYB = np.array([ResRB,ResRB]).T
    ResZYB = np.array([ResZB,ResZB]).T

    ResRXM = np.array([np.unique(M2.Knots[0,:])[:-1]+Eps, np.unique(M2.Knots[0,:])[1:]-Eps]).T
    ResZXM = np.array([np.unique(M2.Knots[1,:])[:-1]+Eps, np.unique(M2.Knots[1,:])[1:]-Eps]).T
    ResRYM = np.array([ResRM,ResRM]).T
    ResZYM = np.array([ResZM,ResZM]).T

    ax2.plot(ResZYB.flatten(), ResZXB.flatten(), label=Leg+' Z-Res Bckgrd', **Bckdict)
    ax2.plot(ResZYM.flatten(), ResZXM.flatten(), label=Leg+' Z-Res Mesh', **Mshdict)
    ax3.plot(ResRXB.flatten(), ResRYB.flatten(), label=Leg+' R-Res Bckgrd', **Bckdict)
    ax3.plot(ResRXM.flatten(), ResRYM.flatten(), label=Leg+' R-Res Mesh', **Mshdict)
