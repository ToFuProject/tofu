...
_DT = abs(_LT[1] - _LT[0])
_RES = np.logspace(_LRES[0], _LRES[1], _LRES[2] * _DRES + _DRES + 1, base=10)
_NUMTOT = _LLOS[2] * _DLOS + _DLOS + 1
_NLOS = np.round(np.logspace(_LLOS[0], _LLOS[1], _NUMTOT, base=10))
                ...
                for tt in range(nnt):
                    ...
                    for rr in range(nrep):
                        ...
                        try:
                            if func is None:
                                t0 = dtm.datetime.now()
                                _ = cam.calc_signal_from_Plasma2D(
                                    plasma,
                                    quant=quant,
                                    ref1d=ref1d,
                                    ref2d=ref2d,
                                ...
                                tm_now = dtm.datetime.now()
                                dt[rr] = (tm_now - t0).total_seconds()
                            else:
                                t0 = dtm.datetime.now()
                                _ = cam.calc_signal(
                                    func,
...
if __name__ == "__main__":
    ...
    msg = """ Launch benchmark for _GG.LOS_calc_signal
    ...
    parser.add_argument(
        ...
        "--nameappend",
        type=str,
        help="str to be appended to the name",
        required=False,
        default=None,
    )
    parser.add_argument(
