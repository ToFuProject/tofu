...
def Fig01(Deriv=0, a4=False, save=True):
    ...
    for ii in range(0,len(Deg)):
        ...
        if ii in [2,3]:
            La[ii].set_xticklabels([r"$x_{0}$",r"$x_{1}$",r"$x_{2}$",r"$x_{3}$",r"$x_{4}$"], size=15)
        else:
    ...
    if save:
        f.savefig('/afs/ipp-garching.mpg.de/home/d/didiv/Documents/Notes/Bsplines_Integrals/BSplines_GeneralExpression_Deriv'+str(Deriv)+'.pdf', format='pdf')
    return La
